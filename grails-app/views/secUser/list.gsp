			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="fullname" title="${message(code: 'secUser.fullname.label', default: 'Fullname')}" />
					
						<g:sortableColumn property="departement" title="${message(code: 'secUser.departement.label', default: 'Departement')}" />
				...
				<g:each in="${secUserInstanceList}" status="i" var="secUserInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: secUserInstance, field: "fullname")}</td>
					
						<td>${fieldValue(bean: secUserInstance, field: "departement")}</td>
